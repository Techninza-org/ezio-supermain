...
const VendorKycDetails = () => {
  ...
  async function getDetails() {
    const res = await axios.post(
      `${import.meta.env.VITE_BASE_URL}superAdmin/kyc/vendor`,
  ...
  async function handleAccept() {
    const res = await axios.put(
      `${import.meta.env.VITE_BASE_URL}superAdmin/accept`,
      { host_id: Number(host_id) },
      {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  async function handleReject() {
    const res = await axios.put(
      `${import.meta.env.VITE_BASE_URL}superAdmin/reject`,
      { host_id: Number(host_id) },
      {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      <div className="wrapper d-flex flex-column min-vh-100">
        ...
        <div className="body flex-grow-1 m-4">
          ...
          <div className="d-flex gap-4 mt-5">
            ...
            <CButton color="secondary" onClick={() => handleReject()} className="ml-4">
              Reject
            </CButton>
          </div>
        </div>
